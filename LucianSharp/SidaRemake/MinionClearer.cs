...
namespace LucianSharp.SidaRemake
{
    class MinionClearer
    {
        ...
        public static Obj_AI_Base findKillable()
        {
            ...
            foreach (var enemyMinion in EnemyMinions)
            {
                if(!LXOrbwalker.InAutoAttackRange(enemyMinion))
                    continue;
        ...
        public static Obj_AI_Base getMinUnderTower()
        {
            foreach (var tAttack in DtsHealthPrediction.ActiveAttacksTower)
            {
                ...
                if (enem.IsValidTarget() && enem is Obj_AI_Minion && enem.IsEnemy && LXOrbwalker.InAutoAttackRange(enem))
                {
        ...
        public static Obj_AI_Base getTowerTarget()
        {
            foreach (var tAttack in DtsHealthPrediction.ActiveAttacksTower)
            {
                ...
                if (enem.IsValidTarget() && enem is Obj_AI_Minion && enem.IsEnemy && LXOrbwalker.InAutoAttackRange(enem))
                { 
                    var time = LXOrbwalker.GetCurrentWindupTime() + 1000* player.Distance(enem)/ player.BasicAttack.MissileSpeed;
