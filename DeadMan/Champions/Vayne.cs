        ...
        public override void DeathWalker_AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            var useQ =
                GetValue<bool>(
                    "UseQ" +
                    (ComboActive
                        ? DeathWalker.CurrentMode == DeathWalker.Mode.Combo ? "C" : ""
                        : DeathWalker.CurrentMode == DeathWalker.Mode.LaneClear ? "H" : ""));
            if (unit.IsMe && useQ)
                Q.Cast(Game.CursorPos);
        }
                        ...
                        ObjectManager.Get<Obj_AI_Hero>()
                            .Where(
                                enemy =>
                                    ...
                                    enemy.IsValidTarget(
                                        (Q.IsReady() ? Q.Range : 0) +
                                        DeathWalker.getRealAutoAttackRange(ObjectManager.Player)))
                            .FirstOrDefault(
