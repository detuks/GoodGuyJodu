...
namespace ARAMDetFull
{
    class ARAMSimulator
    {
        ...
        private static void TowerAttackOnCreate(GameObject sender, EventArgs args)
        {
            if (sender.IsValid<MissileClient>())
            {
                ...
                if (missile.SpellCaster.IsValid<Obj_AI_Turret>() && missile.SpellCaster.IsEnemy &&
                    missile.Target.IsValid<Obj_AI_Hero>() && missile.Target.IsAlly)
                {
                    ...
                    else if (((Obj_AI_Hero) missile.Target).Distance(turret) < 700)
                    {
        ...
        public static Obj_AI_Hero deepestAlly = player;
