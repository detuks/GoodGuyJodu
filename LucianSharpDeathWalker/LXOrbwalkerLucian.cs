   ...
   /* public class DeathWalker
    {
        ...
        public static void AddToMenu(Menu menu)
        {
            ...
            var menuDrawing = new Menu("Drawing", "DeathWalker_Draw");
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_AARange", "AA Circle").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_AARange_Enemy", "AA Circle Enemy").SetValue(new Circle(true, Color.Pink)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_Holdzone", "Holdzone").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_MinionHPBar", "Minion HPBar").SetValue(new Circle(true, Color.Black)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_MinionHPBar_thickness", "^ HPBar Thickness").SetValue(new Slider(1, 1, 3)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_hitbox", "Show HitBoxes").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_Lasthit", "Minion Lasthit").SetValue(new Circle(true, Color.Lime)));
            menuDrawing.AddItem(new MenuItem("DeathWalker_Draw_nearKill", "Minion nearKill").SetValue(new Circle(true, Color.Gold)));
            menu.AddSubMenu(menuDrawing);

            var menuMisc = new Menu("Misc", "DeathWalker_Misc");
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_Holdzone", "Hold Position").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_Farmdelay", "Farm Delay").SetValue(new Slider(0, 200, 0)));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_ExtraWindUp", "Extra Winduptime").SetValue(new Slider(80, 200, 0)));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_AutoWindUp", "Autoset Windup").SetValue(false));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_Priority_Unit", "Priority Unit").SetValue(new StringList(new[] { "Minion", "Hero" })));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_Humanizer", "Humanizer Delay").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_AllMovementDisabled", "Disable All Movement").SetValue(false));
            menuMisc.AddItem(new MenuItem("DeathWalker_Misc_AllAttackDisabled", "Disable All Attacks").SetValue(false));

            menu.AddSubMenu(menuMisc);

            var menuMelee = new Menu("Melee", "DeathWalker_Melee");
            menuMelee.AddItem(new MenuItem("DeathWalker_Melee_Prediction", "Movement Prediction").SetValue(false));
            menu.AddSubMenu(menuMelee);

            var menuModes = new Menu("Orbwalk Mode", "DeathWalker_Modes");
            {
                var modeCombo = new Menu("Combo", "DeathWalker_Modes_Combo");
                ...
                var modeHarass = new Menu("Harass", "DeathWalker_Modes_Harass");
                ...
                var modeLaneClear = new Menu("LaneClear", "DeathWalker_Modes_LaneClear");
                ...
                var modeLaneFreeze = new Menu("LaneFreeze", "DeathWalker_Modes_LaneFreeze");
                ...
                var modeLasthit = new Menu("LastHit", "DeathWalker_Modes_LastHit");
                ...
                var modeFlee = new Menu("Flee", "DeathWalker_Modes_Flee");
        ...
        public static void Orbwalk(Vector3 goalPosition, Obj_AI_Base target)
        {
            ...
            if (MyHero.IsMelee() && target != null && target.Distance(MyHero) < GetAutoAttackRange(MyHero, target) &&
                Menu.Item("DeathWalker_Melee_Prediction").GetValue<bool>() && target is Obj_AI_Hero && Game.CursorPos.Distance(target.Position) < 300)
            {
        ...
        private static void MoveTo(Vector3 position, float holdAreaRadius = -1)
        {
            ...
            if (!CanMove())
                return;
            if (holdAreaRadius < 0)
                holdAreaRadius = Menu.Item("DeathWalker_Misc_Holdzone").GetValue<Slider>().Value;
        ...
        private static bool IsAllowedToMove()
        {
            if (!_movement)
                return false;
            if (Menu.Item("DeathWalker_Misc_AllMovementDisabled").GetValue<bool>())
                return false;
        ...
        private static bool IsAllowedToAttack()
        {
            if (!_attack)
                return false;
            if (Menu.Item("DeathWalker_Misc_AllAttackDisabled").GetValue<bool>())
                return false;
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (Menu.Item("DeathWalker_Draw_AARange").GetValue<Circle>().Active)
            {
                Utility.DrawCircle(MyHero.Position, GetAutoAttackRange(), Menu.Item("DeathWalker_Draw_AARange").GetValue<Circle>().Color);
            }

            if (Menu.Item("DeathWalker_Draw_AARange_Enemy").GetValue<Circle>().Active ||
                Menu.Item("DeathWalker_Draw_hitbox").GetValue<Circle>().Active)
            {
                foreach (var enemy in AllEnemys.Where(enemy => enemy.IsValidTarget(1500)))
                {
                    if (Menu.Item("DeathWalker_Draw_AARange_Enemy").GetValue<Circle>().Active)
                        Utility.DrawCircle(enemy.Position, GetAutoAttackRange(enemy, MyHero), Menu.Item("DeathWalker_Draw_AARange_Enemy").GetValue<Circle>().Color);
                    if (Menu.Item("DeathWalker_Draw_hitbox").GetValue<Circle>().Active)
                        Utility.DrawCircle(enemy.Position, enemy.BoundingRadius, Menu.Item("DeathWalker_Draw_hitbox").GetValue<Circle>().Color);
                }
            }
            ...
            if (Menu.Item("DeathWalker_Draw_Holdzone").GetValue<Circle>().Active)
            {
                Utility.DrawCircle(MyHero.Position, Menu.Item("DeathWalker_Misc_Holdzone").GetValue<Slider>().Value, Menu.Item("DeathWalker_Draw_Holdzone").GetValue<Circle>().Color);
            }

            if (Menu.Item("DeathWalker_Draw_MinionHPBar").GetValue<Circle>().Active ||
                Menu.Item("DeathWalker_Draw_Lasthit").GetValue<Circle>().Active ||
                Menu.Item("DeathWalker_Draw_nearKill").GetValue<Circle>().Active)
            {
                    ...
                    if (Menu.Item("DeathWalker_Draw_MinionHPBar").GetValue<Circle>().Active)
                    {
                        for (var i = 1; i < attackToKill; i++)
                        {
                            ...
                            Drawing.DrawLine(
                                ...
                                Menu.Item("DeathWalker_Draw_MinionHPBar_thickness").GetValue<Slider>().Value,
                                Menu.Item("DeathWalker_Draw_MinionHPBar").GetValue<Circle>().Color);
                        }
                    }
                    if (Menu.Item("DeathWalker_Draw_Lasthit").GetValue<Circle>().Active &&
                        minion.Health <= MyHero.GetAutoAttackDamage(minion, true))
                        Utility.DrawCircle(minion.Position, minion.BoundingRadius, Menu.Item("DeathWalker_Draw_Lasthit").GetValue<Circle>().Color);
                    else if (Menu.Item("DeathWalker_Draw_nearKill").GetValue<Circle>().Active &&
                             minion.Health <= MyHero.GetAutoAttackDamage(minion, true) * 2)
                        Utility.DrawCircle(minion.Position, minion.BoundingRadius, Menu.Item("DeathWalker_Draw_nearKill").GetValue<Circle>().Color);
                }
            }
        }
        ...
        public static Obj_AI_Base GetPossibleTarget()
        {
            ...
            if (Menu.Item("DeathWalker_Misc_Priority_Unit").GetValue<StringList>().SelectedIndex == 1 &&
        ...
        {
            if ("lucianpassiveattack" == name.ToLower() && CurrentMode != Mode.Combo)
                return true;
        ...
        public int canAttackIn()
        {
            return (int)(_lastAATick + MyHero.AttackDelay*1000) - (Environment.TickCount + Game.Ping/2 + 25);
        }

        public static bool CanMove()
        {
            var extraWindup = Menu.Item("DeathWalker_Misc_ExtraWindUp").GetValue<Slider>().Value;
        ...
        private static int FarmDelay()
        {
            ...
            if (MyHero.ChampionName == "Azir")
                ret += 125;
            return Menu.Item("DeathWalker_Misc_Farmdelay").GetValue<Slider>().Value + ret;
        }
        ...
        {
            if (!Menu.Item("DeathWalker_Misc_AutoWindUp").GetValue<bool>())
                return;
            var additional = 0;
            if (Game.Ping >= 100)
            ...
            if (windUp < 40)
                windUp = 40;
            Menu.Item("DeathWalker_Misc_ExtraWindUp").SetValue(windUp < 200 ? new Slider(windUp, 200, 0) : new Slider(200, 200, 0));
        }

        public static int GetCurrentWindupTime()
        {
            return Menu.Item("DeathWalker_Misc_ExtraWindUp").GetValue<Slider>().Value;
        }
        ...
        public static float GetAutoAttackRange(Obj_AI_Base source = null, Obj_AI_Base target = null)
        {
            if (source == null)
                source = MyHero;
            var ret = source.AttackRange + MyHero.BoundingRadius-20;
            if (target != null)
                ret += target.BoundingRadius;
            return ret;
        }

        public static bool InAutoAttackRange(Obj_AI_Base target, bool useE = false)
        {
            if (target == null)
                return false;
            var myRange = GetAutoAttackRange(MyHero, target) + ((useE && Lucian.E.IsReady())?425:0);
